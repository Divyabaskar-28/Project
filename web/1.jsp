...
<html lang="en">
    ...
    <body>
        ...
        <div class="content">
            <div id="form-container" style="margin-left: 80px;">
                ...
                <div class="row mb-4">
                    <div class="col-md-4">
                        ...
                        <select class="form-select" id="childName" onchange="fetchDOB(), displayChildName()" required>
                            ...
                            <%                        String parentEmail = (String) session.getAttribute("uname");
        ...
        <div class="con" id="appointment">
            <%
                String childName = request.getParameter("childName");
                String hospital = request.getParameter("hospital");
                String date = request.getParameter("date");
                String vaccine = request.getParameter("vaccine");
                parentEmail = (String) session.getAttribute("uname");
                ...
                if ("POST".equalsIgnoreCase(request.getMethod())) { // Check if form is submitted
                    ...
                    try {
                        ...
                        String sql = "INSERT INTO book_appointments (child_name, parent_email, hospital, appointment_date, vaccine) VALUES (?, ?, ?, ?, ?)";
                        pst = con.prepareStatement(sql);
                        pst.setString(1, childName);
                        pst.setString(2, parentEmail);
            ...
            <div class="container form-container" style="margin-left: 70px; width:600px; margin-left: 480px; position:absolute; top:70px;">
                ...
                <form id="appointmentForm" action="BookAppointment.jsp" method="post" onsubmit="return bookAppointment(event)">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="selectedChild" readonly>
                    </div>
                    ...
                    <div class="row g-3">
                            ...
                            <select id="hospital" name="hospital" class="form-select" required>
                                ...
                                <%
                                    ...
                                    try {
                                        ...
                                        while (rs.next()) {
                                %>
                                <option value="<%= rs.getString("hospital_name")%>" 
                                        <%= request.getParameter("hospital") != null && request.getParameter("hospital").equals(rs.getString("hospital_name")) ? "selected" : ""%>>
                        ...
                        <div class="col-md-6">
                            ...
                            <select id="hospitalBranch" name="hospitalBranch" class="form-select" required>
                                ...
                                <%
                                    // Fetch and display branches for the selected hospital
                                    String selectedHospital = request.getParameter("hospital");
                                    if (selectedHospital != null && !selectedHospital.isEmpty()) {
                                        Connection conBranch = null;
                                        PreparedStatement psBranch = null;
                                        ResultSet rsBranch = null;

                                        try {
                                            conBranch = DriverManager.getConnection("jdbc:mysql://localhost:3306/cvms", "root", "");
                                            psBranch = conBranch.prepareStatement("SELECT DISTINCT branch FROM hospital_details WHERE hospital_name = ?");
                                            psBranch.setString(1, selectedHospital);
                                ...
                                <option value="<%= rsBranch.getString("branch")%>"
                                        <%= request.getParameter("hospitalBranch") != null && request.getParameter("hospitalBranch").equals(rsBranch.getString("branch")) ? "selected" : ""%>>
...
// 2. First, clear previous options
                    ...
                    if (ageInDays >= 0 && ageInDays < 42) {
                        ...
                        currentRowId = "month16_24Row";
                    } else {
                        alert("No scheduled vaccines for this age.");
                        return;
                    }

                    // Mark the current row's status as "Current" ? NEW
                    if (currentRowId) {
                        const currentStatusCell = document.querySelector(`#${currentRowId} .status-cell`);
                        if (currentStatusCell && currentStatusCell.textContent !== "Completed") {
                            currentStatusCell.textContent = "Current";
                        }
                    }
...
<!-- Now preload ALL hospital-branch data into JS -->
<script>
